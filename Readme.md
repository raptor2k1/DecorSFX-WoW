...
* **Spam Clicking Audio Drop/Doubling:** Rapid clicking in succession may lead to successive sounds being omitted or a file playing multiple times on a single interact. A brief delay is built-in to prevent audio stacking from getting too severe, and you can adjust this cooldown interval at the bottom of `InteractionPlayer.lua`. The default SFX playback cooldown settings should prevent multiple clip plays on a single interact, even if spam clicking, but I cannot necessarily guarantee this holds true on all player PC's.
